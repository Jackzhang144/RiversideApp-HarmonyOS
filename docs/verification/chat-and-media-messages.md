...
结果：主 HAP 与测试 HAP 均构建成功；设备测试 `191/191` 通过，
...
- `upload://` 图片地址、自定义 Emoji、服务端 tracking 未读数与频道动作权限；
- 频道消息分页、搜索结果目标消息窗口、真实消息 ID 已读上报；
- 私聊、发送、Reaction、删除、恢复、搜索和离开频道接口契约；
- 上传失败和发送失败时保留可重试草稿，不创建伪消息；
- 发送只接受服务端 `message_id` 确认，并用目标消息查询同步真实消息；
- 切换频道后忽略旧发送响应，已确认但暂未同步的消息不会被重复发送；
